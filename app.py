...
async def dashboard(request: web.Request) -> web.Response:
    ...
    conn.close()
    if len(courses) == 1:
        raise web.HTTPFound(f"/courses/{courses[0]['id']}")
    html = render_template('dashboard.html', courses=courses, session={'user_id': True}, request_path=str(request.rel_url))
    return web.Response(text=html, content_type='text/html')
...
async def course_create_get(request: web.Request) -> web.Response:
    ...
    await ensure_logged(request)
    html = render_template('course_edit.html', course=None, session={'user_id': True}, request_path=str(request.rel_url))
    return web.Response(text=html, content_type='text/html')
...
async def course_edit_get(request: web.Request) -> web.Response:
    ...
    html = render_template('course_edit.html', course=course, session={'user_id': True}, request_path=str(request.rel_url))
    return web.Response(text=html, content_type='text/html')
...
async def view_course(request: web.Request) -> web.Response:
    ...
    if not course:
        raise web.HTTPNotFound()
    html = render_template('modules.html', course=course, modules=modules, session={'user_id': True}, request_path=str(request.rel_url))
    return web.Response(text=html, content_type='text/html')
...
async def module_create_get(request: web.Request) -> web.Response:
    ...
    await ensure_logged(request)
    course_id = int(request.match_info['course_id'])
    html = render_template('module_edit.html', module=None, course_id=course_id, session={'user_id': True}, request_path=str(request.rel_url))
    return web.Response(text=html, content_type='text/html')
...
async def module_edit_get(request: web.Request) -> web.Response:
    ...
    if not module:
        raise web.HTTPNotFound()
    html = render_template('module_edit.html', module=module, course_id=module['course_id'], session={'user_id': True}, request_path=str(request.rel_url))
    return web.Response(text=html, content_type='text/html')
...
async def logo_get(request: web.Request) -> web.Response:
    ...
    await ensure_logged(request)
    html = render_template('update_logo.html', logo='/static/logo.png', session={'user_id': True}, request_path=str(request.rel_url))
    return web.Response(text=html, content_type='text/html')
